			...
			<div class="user-settings__fields">
				...
				<div class="user-settings__color field">
					<label for="theme-color">{{
						`${$t("common.theme")} ${$t("common.color").toLowerCase()}`
					}}</label>
					<hr />
					<input
						id="theme-color"
				...
				<div class="user-settings__language field">
					...
					<select
						...
						@input="setLanguage"
					>
						<option v-for="lang in languages" :value="lang" :key="lang.key">
							{{ lang.name }}
						</option>
					</select>
				</div>
...
export default class SettingsCard extends Vue {
	...
	public mounted() {
		this.selectedLanguage =
			this.languages.find((l) => l.key == this.user.settings?.languageKey) ??
